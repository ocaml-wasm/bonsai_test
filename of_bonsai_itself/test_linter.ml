...
let%expect_test "map2_unfolded_constant_warnings" =
  ...
  in
  test_lint c;
  [%expect {| lib/bonsai_test/of_bonsai_itself/test_linter.ml:2:4: unfolded constant |}]
;;
...
let%expect_test "map2_two_unfolded_constants_warnings" =
  ...
  in
  test_lint doubled;
  [%expect {| lib/bonsai_test/of_bonsai_itself/test_linter.ml:7:4: unfolded constant |}]
;;
...
let%expect_test "cutoff_unfolded_constants_warnings" =
  ...
  test_lint cutoff;
  [%expect {| lib/bonsai_test/of_bonsai_itself/test_linter.ml:1:23: unfolded constant |}]
;;
...
let%expect_test "sm1_with_const_input_gets_warning" =
  ...
  [%expect
    {| lib/bonsai_test/of_bonsai_itself/test_linter.ml:2:4: state_machine1 can be optimized to a state_machine0 |}]
;;
...
let%expect_test "map2_with_unfolded_constants_and_sm1_with_const_input_both_warned" =
  ...
  [%expect
    {|
    lib/bonsai_test/of_bonsai_itself/test_linter.ml:3:6: state_machine1 can be optimized to a state_machine0
    lib/bonsai_test/of_bonsai_itself/test_linter.ml:13:6: unfolded constant
    |}]
;;
