...
module Degenerate_svar = struct
  ...
  let create () =
    ...
    let eff = Effect.Expert.of_fun ~f:(fun ~callback -> setter := callback) in
    (fun v -> !setter v), eff
  ;;
end

let both_not_parallel ?(here = Stdlib.Lexing.dummy_pos) a b =
...
let%expect_test "Effect.both_parallel degenerate" =
  bisimulate_both_boths ~f:(fun both ~expect_diff ->
    ...
    [%expect
      ...
      |}];
    fill_eff2 ();
    (* This shows the change in behavior between the previous "both"/"all" and the new
       "both_parallel". *)
    expect_diff
