...
module%test [@name "computation watcher"] _ = struct
  ...
  let%expect_test "BUG: [Fetch node] dynamic lookup computation watcher" =
    ...
    [%expect
      {|
      --------------------------------
      Watched computation updated due to Named node at [:0:-1]
      ...
      Updated computation depended on at:
       - lib/bonsai_test/of_bonsai_itself/test_cont_bonsai.ml:LINE:COL
      --------------------------------
      2 watched computations updated due to Named node at [:0:-1]
      ...
      (by (
        ...
        (into (
          ...
          (from (
            ...
            (into (
              Sub
              (from (Return (value (Mapn (inputs ((Named (uid (Test 11)))))))))
              (via (Test 13))
              (into (Lifecycle (value (Named (uid (Test 13))))))))))
          (via (Test 14))
          (into (Return (value (Named (uid (Test 3)))))))))))
    |}]
;;
